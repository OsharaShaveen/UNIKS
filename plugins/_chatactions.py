...
try:
    ...
    detector = None
from . import LOG_CHANNEL, LOGS, asst, get_string, types, udB, UNIKS_bot
from ._inline import something


@UNIKS_bot.on(events.ChatAction())
...
async def DummyHandler(ult):
    ...
    if (
        ...
        if not user.bot:
            ...
            try:
                await UNIKS_bot(GetParticipantRequest(int(joinchat), user.id))
            except UserNotParticipantError:
                await UNIKS_bot.edit_permissions(
                    ult.chat_id, user.id, send_messages=False
                )
                res = await UNIKS_bot.inline_query(
    ...
    if ult.user_joined or ult.added_by:
        ...
        # gbans and @UNIKSBans checks
        if udB.get_key("UNIKS_BANS"):
            try:
                is_banned = await async_searcher(
                    "https://bans.UNIKS.tech/api/status",
                    json={"userId": user.id},
                    post=True,
                    re_json=True,
                )
                if is_banned["is_banned"]:
                    ...
                    await ult.client.send_message(
                        chat.id,
                        "**@UNIKSBans:** Banned user detected and banned!\n`{}`.\nBan reason: {}".format(
                            str(is_banned),
                            is_banned["reason"],
                        ),
                    )
...
@UNIKS_bot.on(events.NewMessage(incoming=True))
...
@UNIKS_bot.on(events.Raw(types.UpdateUserName))
