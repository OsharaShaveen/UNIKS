...
@UNIKS_cmd(pattern="fsub( (.*)|$)", admins_only=True, groups_only=True)
async def addfor(e):
    ...
    UNIKS_bot.add_handler(force_sub, events.NewMessage(incoming=True))


@UNIKS_cmd(pattern="remfsub$")
async def remor(e):
...
async def fcall(e):
    match = e.pattern_match.group(1).strip()
    spli = match.split("_")
    user = await UNIKS_bot.get_entity(int(spli[0]))
    cl = await UNIKS_bot.get_entity(int(spli[1]))
    text = f"Hi {inline_mention(user)}, You Need to Join"
    text += f" {cl.title} in order to Chat in this Group."
    if not cl.username:
        el = (await UNIKS_bot(ExportChatInviteRequest(cl))).link
    else:
...
async def diesoon(e):
    ...
    try:
        values = await UNIKS_bot(GetParticipantRequest(int(spli[1]), int(spli[0])))
    ...
    await UNIKS_bot.edit_permissions(
...
async def force_sub(ult):
    ...
    if count in range(2, 11):
        return
    try:
        await UNIKS_bot.get_permissions(int(joinchat), user.id)
        return
    except UserNotParticipantError:
        pass
    if isinstance(user, Channel):
        try:
            ...
            LOGS.exception(er)
    try:
        await UNIKS_bot.edit_permissions(ult.chat_id, user.id, send_messages=False)
    except ChatAdminRequiredError:
        return
    except Exception as e:
        await ult.delete()
        LOGS.info(e)
    res = await UNIKS_bot.inline_query(asst.me.username, f"fsub {user.id}_{joinchat}")
