...
@UNIKS_cmd(pattern="kickme$", fullsudo=True)
async def leave(ult):
    me = asst.me if ult.client._bot else UNIKS_bot.me
...
@UNIKS_cmd(pattern="paste( (.*)|$)", manager=True, allow_all=True)
async def _(event):
    try:
...
async def telegraphcmd(event):
    match = event.pattern_match.group(1).strip() or "UNIKS"
    reply = await event.get_reply_message()
    if not reply:
...
@UNIKS_cmd(pattern="suggest( (.*)|$)", manager=True)
