...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/TeamUNIKS/UNIKS"),
        Button.url("• Support •", url="https://t.me/httu2I8W94e9Lo3MmJl"),
    ],
]
...
async def inline_handler(event):
    ...
    if INLINE_PIC:
        ...
        result = await event.builder.article(
            title="UNIKS Help Menu", text=text, buttons=_main_help_menu
        )
...
async def _(e):
    button = [
        ...
        [
            Button.switch_inline(
                "Piston Eval",
                query="run javascript console.log('Hello UNIKS')",
                same_peer=True,
            ),
            Button.switch_inline(
                "OʀᴀɴɢᴇFᴏx🦊",
        ...
        [
            Button.switch_inline(
                "Tᴡɪᴛᴛᴇʀ Usᴇʀ", query="twitter theUNIKS", same_peer=True
            ),
            Button.switch_inline(
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("UNIKS Op", text=txt, link_preview=False, buttons=btn)
    ]
