...
from . import LOGS, asst, eor, events, get_string, udB, UNIKS_bot, UNIKS_cmd
...
async def source(e):
    ...
    if not is_source_channel_added(y):
        ...
        UNIKS_bot.add_handler(autopost_func, events.NewMessage())
    elif is_source_channel_added(y):
        await e.eor(get_string("cha_3"))


@UNIKS_cmd(pattern="dsource( (.*)|$)")
async def dd(event):
...
@UNIKS_cmd(pattern="listdest")
async def list_all(event):
    UNIKS_bot = event.client
    x = await event.eor(get_string("com_1"))
    ...
    for channel in channels:
        name = ""
        try:
            name = get_display_name(await UNIKS_bot.get_entity(int(channel)))
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
