...
from . import LOGS, asst, get_string, inline_mention, UNIKS_bot, UNIKS_cmd
...
@UNIKS_cmd(pattern="cban$", admins_only=True)
...
@UNIKS_cmd(pattern="(list|add|rem)wl( (.*)|$)")
...
if dnd_db.get_dnd_chats():
    UNIKS_bot.add_handler(dnd_func, events.ChatAction(func=lambda x: x.user_joined))
