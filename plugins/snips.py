...
from . import events, get_string, mediainfo, udB, UNIKS_bot, UNIKS_cmd
from ._inline import something


@UNIKS_cmd(pattern="addsnip( (.*)|$)")
async def an(e):
    ...
    UNIKS_bot.add_handler(add_snips, events.NewMessage())


@UNIKS_cmd(pattern="remsnip( (.*)|$)")
async def rs(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    if not wrd:
    ...
    for z in xx:
        ...
        if k:
            ...
            if rep:
                ...
                await UNIKS_bot.send_message(e.chat_id, msg, file=media)
