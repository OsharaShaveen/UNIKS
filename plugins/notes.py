...
@UNIKS_cmd(pattern="addnote( (.*)|$)", admins_only=True)
async def an(e):
    ...
    UNIKS_bot.add_handler(notes, events.NewMessage())


@UNIKS_cmd(pattern="remnote( (.*)|$)", admins_only=True)
async def rn(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
...
@UNIKS_cmd(pattern="listnote$", admins_only=True)
async def lsnote(e):
