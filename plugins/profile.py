...
from . import eod, eor, get_string, mediainfo, UNIKS_cmd
...
@UNIKS_cmd(pattern="setbio( (.*)|$)", fullsudo=True)
async def _(ult):
    ok = await ult.eor("...")
...
@UNIKS_cmd(pattern="setname ?((.|//)*)", fullsudo=True)
async def _(ult):
    ok = await ult.eor("...")
...
@UNIKS_cmd(pattern="setpic$", fullsudo=True)
async def _(ult):
    if not ult.is_reply:
...
@UNIKS_cmd(pattern="delpfp( (.*)|$)", fullsudo=True)
