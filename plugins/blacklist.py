...
from . import events, get_string, udB, UNIKS_bot, UNIKS_cmd


@UNIKS_cmd(pattern="blacklist( (.*)|$)", admins_only=True)
async def af(e):
    ...
    UNIKS_bot.add_handler(blacklist, events.NewMessage(incoming=True))
    await e.eor(get_string("blk_2").format(wrd))


@UNIKS_cmd(pattern="remblacklist( (.*)|$)", admins_only=True)
async def rf(e):
    wrd = e.pattern_match.group(1).strip()
    chat = e.chat_id
    if not wrd:
...
@UNIKS_cmd(pattern="listblacklist$", admins_only=True)
async def lsnote(e):
