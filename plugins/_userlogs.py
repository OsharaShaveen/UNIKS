...
if udB.get_key("TAG_LOG"):
    ...
    @UNIKS_bot.on(events.MessageEdited(func=lambda x: not x.out))
    ...
    async def idk(e):
        ...
        if chat and msg:
            try:
                await UNIKS_bot.send_message(chat, e.message, reply_to=msg)
