...
from . import Redis, eor, get_string, udB, UNIKS_cmd


@UNIKS_cmd(pattern="setdb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@UNIKS_cmd(pattern="deldb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@UNIKS_cmd(pattern="rendb( (.*)|$)", fullsudo=True)
async def _(ult):
    match = ult.pattern_match.group(1).strip()
    if not match:
