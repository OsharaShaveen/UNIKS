...
from . import LOGS, get_string, udB, UNIKS_bot, UNIKS_cmd


@UNIKS_cmd(pattern="autopic( (.*)|$)")
async def autopic(e):
...
if search := udB.get_key("AUTOPIC"):
    ...
    async def autopic_func():
        ...
        file = await UNIKS_bot.upload_file(img)
        await UNIKS_bot(UploadProfilePhotoRequest(file))
        shuffle(ok)
