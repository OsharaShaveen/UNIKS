...
@UNIKS_cmd(pattern="gpromote( (.*)|$)", fullsudo=True)
async def _(e):
...
@UNIKS_cmd(pattern="gdemote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    UNIKS_bot = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await UNIKS_bot.get_entity(user)
        except BaseException:
            return await e.eor(f"`No User Found Regarding {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in UNIKS_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@UNIKS_cmd(pattern="ungban( (.*)|$)", fullsudo=True)
async def _(e):
...
@UNIKS_cmd(pattern="gban( (.*)|$)", fullsudo=True)
async def _(e):
...
@UNIKS_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=True)
...
@UNIKS_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
...
@UNIKS_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
...
@UNIKS_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
...
@UNIKS_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(UNIKS_bot.me)}",
        )
...
@UNIKS_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@UNIKS_cmd(pattern="ungblacklist$")
