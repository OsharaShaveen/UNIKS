...
async def pack_kangish(_):
    ...
    if len(_.text) > 9:
        ...
        _packname = f"UNIKS Kang Pack By {_.sender_id}"
...
async def pyUltroid(args):
    UNIKS_bot = args.client
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await UNIKS_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await UNIKS_bot.download_file(message.media.document, photo)
        if (
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent UNIKS_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with UNIKS_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await UNIKS_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await UNIKS_bot.send_read_acknowledge(conv.chat_id)
        else:
...
async def ultdestroy(event):
    ...
    await event.client.download_media(ult, "UNIKS.tgs")
    xx = await event.eor(get_string("com_1"))
    await bash("lottie_convert.py UNIKS.tgs json.json")
    ...
    file = await con.animated_sticker("json.json", "UNIKS.tgs")
    if file:
        await event.client.send_file(
            event.chat_id,
            file="UNIKS.tgs",
...
async def ultiny(event):
    ...
    im1 = Image.open("resources/extras/UNIKS_blank.png")
    if ik.endswith(".tgs"):
