...
from . import Button, Redis, asst, callback, eod, get_string, UNIKS_bot, UNIKS_cmd
...
async def unmuting(e):
    ...
    user = int(ino[0])
    chat = int(ino[1])
    user_name = (await UNIKS_bot.get_entity(user)).first_name
    chat_title = (await UNIKS_bot.get_entity(chat)).title
    await UNIKS_bot.edit_permissions(chat, user, send_messages=True)
    await e.edit(
