...
async def sketch(e):
    ...
    cv2.imwrite("UNIKS.png", pencil_sketch_IMG)
    await e.reply(file="UNIKS.png")
    await xx.delete()
    os.remove(file)
    os.remove("UNIKS.png")


@UNIKS_cmd(pattern="color$")
async def _(event):
...
async def ultd(event):
    ...
    file = await con.convert(ultt, convert_to="png", outname="ult")
    ult = cv2.imread(file)
    UNIKS = cv2.cvtColor(ult, cv2.COLOR_BGR2GRAY)
    cv2.imwrite("ult.jpg", UNIKS)
...
async def ultd(event):
    ...
    file = await con.convert(ultt, convert_to="png", outname="ult")
    ult = cv2.imread(file)
    UNIKS = cv2.GaussianBlur(ult, (35, 35), 0)
    cv2.imwrite("ult.jpg", UNIKS)
    await event.client.send_file(
        event.chat_id,
        "ult.jpg",
...
async def ultd(event):
    ...
    UNIKS = cv2.cvtColor(dan, cv2.COLOR_HSV2BGR)
    cv2.imwrite("ult.jpg", UNIKS)
    await event.client.send_file(
        event.chat_id,
        "ult.jpg",
...
async def pixelator(event):
    ...
    await msg.respond("• Pixelated by UNIKS", file="output.jpg")
    await msg.delete()
