...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for @SLSVF:",
        "buttons": [
            [
    ...
    "oofdm": {
        "text": "About [Dual Mode](https://t.me/UNIKSUpdates/18)",
        "buttons": [
            [
...
async def changes(okk):
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• UNIKS Userbot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
    ...
    if len(changelog_str) > 1024:
        ...
        with open("UNIKS_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await okk.edit(
            get_string("upd_5"),
            file="UNIKS_updates.txt",
            buttons=button,
        )
        remove("UNIKS_updates.txt")
        return
    await okk.edit(
        changelog_str,
        buttons=button,
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with UNIKS Userbot.`",
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET by [this](https://t.me/UNIKSUpdates/37)",
        buttons=[
            [
