...
**UNIKS [v{UNIKS_version}](https://github.com/TeamUNIKS/UNIKS), powered by @TeamUNIKS**
"""
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(UNIKS_bot.me)
    )
    if custom_info:
        msg += "\n\n• Powered by **@SLSVF**"
    await event.edit(
...
async def UNIKS(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(UNIKS_bot.me)
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey there {mention}, this is UNIKS Assistant of {me}!\n\n{ok}",
